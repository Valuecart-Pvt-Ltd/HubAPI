      ...
      <table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;border-radius:14px;overflow:hidden">
        <tr>
          <td style="background:linear-gradient(135deg,#2D1B69,#5B21B6);padding:28px 32px;text-align:center">
            <div style="font-size:22px;font-weight:700;color:#ffffff;letter-spacing:-0.3px">📋 Minutes of Meeting</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.7);margin-top:4px">Finalized · Karya</div>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="background:#12102A;padding:16px 32px;border:1px solid #2D2A50;border-top:none;text-align:center">
            <p style="margin:0;font-size:11px;color:#6D5FA6">Automated notification from <strong style="color:#A78BFA">Karya</strong></p>
          </td>
        </tr>
      </table>
    </td></tr>
      ...
      <table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;border-radius:14px;overflow:hidden">
        <tr>
          <td style="background:linear-gradient(135deg,#1E3A5F,#1D4ED8);padding:28px 32px;text-align:center">
            <div style="font-size:22px;font-weight:700;color:#ffffff">📅 Meeting Reminder</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.7);margin-top:4px">In 2 days · Karya</div>
          </td>
        </tr>
        <tr>
          <td style="background:#1A1830;padding:24px 32px;border-left:1px solid #2D2A50;border-right:1px solid #2D2A50">
        ...
        <tr>
          <td style="background:#12102A;padding:16px 32px;border:1px solid #2D2A50;border-top:none;text-align:center">
            <p style="margin:0;font-size:11px;color:#6D5FA6">Automated reminder from <strong style="color:#A78BFA">Karya</strong></p>
          </td>
        </tr>
      </table>
    </td></tr>
...
async function sendReminderEmail(opts) {
  ...
  await transporter.sendMail({
    from:    `"Karya" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
    to:      recipients.join(', '),
...
async function sendMomFinalizedEmail(opts) {
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.warn('[email] SMTP not configured — skipping MOM email')
    return
  }
...
async function sendEventInviteEmail(opts) {
...
<body style="margin:0;padding:0;background:#0F0D1F;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:560px;margin:32px auto;background:#1A1830;border-radius:12px;overflow:hidden;border:1px solid #2D2A50">
    ...
    <div style="padding:28px 32px">
      ...
      <p style="margin:20px 0 0;font-size:12px;color:#4A4270">A calendar invite (.ics) is attached.</p>
    </div>
  </div>
</body>
</html>`
